...
OVERRIDE_STATISTICS = {"age": {"type": "continuous", "stats": {"mean": 45, "std_dev": 15, "min": 10}}}
...
def main(cfg, n_observations=None):
    ...
    for src_file in tqdm(src_files):
        ...
        for year in years:
            ...
            generator.save(
                original_root=cfg["ORIGINAL_ROOT"], new_root=cfg["NEW_ROOT"], data=data, replace=replacement_dict
            )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        format="%(asctime)s %(name)s %(levelname)s: %(message)s", datefmt="%Y-%m-%d %H:%M:%S", level=logging_level
    )
