...
MODEL_SAVE_DIR = Path(ROOT, "models_25-07-25")  # Placeholder
PRETRAINED_MODEL_ROOT = Path("/gpfs/ostor/ossc9424/data/llm/runs-pipeline-steps/D3-models/")
MODEL_NAMES = [
    "medium-random",
    "BASE-random",
    "BASE-event",
...
def main() -> None:
    ...
    for task in TASKS:
        ...
        for target_col, (target_type, num_outputs) in task["target_column"].items():
            for model_name, lr in itertools.product(MODEL_NAMES, LR):
                ...
                cfg.update(
                    {
                        ...
                        "result_dir": str(
                            Path(RESULT_DIR, task_file, model_name, f"lr{lr:.0e}_bs{BATCH_SIZE}")
                        ),
                        "model_save_dir": str(
                            Path(MODEL_SAVE_DIR, task_file, model_name, f"lr{lr:.0e}_bs{BATCH_SIZE}")
                        ),
                        'pretrained_model_path': str(PRETRAINED_MODEL_ROOT / model_name / 'model.ckpt'),
                        'pretrained_model_hparams': str(PRETRAINED_MODEL_ROOT / model_name / 'hparams.yaml'),
                        'EARLY_STOP_PATIENCE': 5 if task_file == 'liss_filtered' else 2,
                        'MAX_EPOCHS':  20 if task_file == 'liss_filtered' else 4,  
                        'balance_dataset': target_type != 'numeric'
                    }
                )
                ...
                cfg_fname = cfg_dir / f"lr{lr:.0e}_bs{BATCH_SIZE}_{target_col}.json"
