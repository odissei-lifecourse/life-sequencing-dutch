# finetune.py
# ---------------------------------------------------------------------
# Fine‑tune a pretrained encoder‑only LM for downstream categorical/
# numeric on 1-N GPUs (single node) in a style consistent with
# pretrain.py
# ---------------------------------------------------------------------
import argparse
import logging
...
REQ_KEYS = [
    ...
    "batch_size",
    "epochs",
    "num_targets", # must be 1 for numeric tasks at the moment
    "learning_rate",
]
...
def _integrity_check(hp):
    if not (
        ("label_file" in hp)
        or ("train_label_file" in hp and "val_label_file" in hp)
    ):
        raise ValueError("Need either 'label_file' or both 'train_label_file' + 'val_label_file'")
    missing = [k for k in REQ_KEYS if k not in hp]
    if missing:
        raise ValueError(f"Missing required hparams: {', '.join(missing)}")
    if hp['num_targets'] != 1 and hp['task_type'] == 'numeric':
        raise ValueError(f"num_targets = {hp['num_targets']} is not okay for numeric tasks. It must be 1")
    if hp['task_type'] not in ['numeric', 'binary', 'categorical']:
        raise ValueError(f"task_type = {hp['task_type']} is not supported. Must be either numeric, binary, or categorical")


def _with_defaults(hp):
    for k, v in DEFAULT_VALS.items():
        hp.setdefault(k, v)
    if 'val' not in hp['monitor']:
        if hp['monitor'] not in ['auc', 'acc', 'mcc', 'mae', 'mse', 'r2']:
            raise ValueError(f"Monitor = {hp[monitor]} is not supported!")
        hp['monitor'] = f"val_{hp['monitor']}_epoch"
    return hp
...
# ───────────────────────── callbacks (ckpt) ───────────────────────────
def get_callbacks(ckpt_dir: str, monitor: str):
...
def get_dataloaders(hp):
    if "train_label_file" in hp and "val_label_file" in hp:
        train_label_file = hp["train_label_file"]
        val_label_file   = hp["val_label_file"]
        train_val_split  = 0.0   # give train_ds every row, filter via label file
        val_val_split    = 1.0   # give val_ds every row, filter via label file
    else:
        train_label_file = hp["label_file"]
        val_label_file   = hp["label_file"]
        train_val_split  = hp.get("val_split", 0.1)
        val_val_split    = train_val_split

    train_ds = FineTuneLazyDataset(
        h5_file_path=hp["sequence_encoded"],
        train_file_path=train_label_file,
        phase="train",
    ...
    val_ds = FineTuneLazyDataset(
        h5_file_path=hp["sequence_encoded"],
        train_file_path=val_label_file,
        phase="validation",
        task_type=hp["task_type"],
        target_col=hp["target_col"],
        val_split=val_val_split,   # ← now 1.0 when using a separate file
        return_sequence_id=True,
    )
...
def finetune(hp):
    ...
    callbacks = get_callbacks(ckpt_dir, hp['monitor'])
